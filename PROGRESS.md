...
- [ ] **1.5** `apps/worker/src/engine/funding.ts` — read latest tick per (venue,symbol), compute directed spreads, upsert `opportunities`. (deps: 1.1, 1.2, 1.3, 1.4) → verify: run worker 30s; `SELECT COUNT(*) FROM opportunities WHERE category='funding'` > 0
- [ ] **1.6** `apps/web/app/api/opportunities/route.ts` (GET, filter by `?cat=`, 5s in-memory cache). (deps: 0.9, 0.11) → verify: `curl localhost:3000/api/opportunities?cat=funding` returns JSON array
- [ ] **1.7** `apps/web/app/(tabs)/funding/page.tsx` + `OpportunityRow` + `useSWR` hook. (deps: 1.6) → verify: /funding renders rows; manual "Updated Xs ago" counter advances
...
- [ ] **5.2** `apps/worker/src/adapters/lido/wait-time.ts` — fetch wq-api.lido.fi. (deps: 0.5) → verify: probe prints wait days for 1 ETH and 1000 ETH amounts
...
- [ ] **7.2** `apps/web/app/components/UpdatedAgo.tsx`. (deps: 0.9) → verify: counter advances 1s/s, resets on poll
...
- [ ] **7.5** Footer disclosure component (data sources per tab, not-investment-advice). (deps: 0.9) → verify: visible at bottom of every tab
